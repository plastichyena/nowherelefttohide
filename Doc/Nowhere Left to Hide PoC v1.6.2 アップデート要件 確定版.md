# Nowhere Left to Hide PoC v1.6.2 アップデート要件 確定版

- ステータス: **アップデート要件確定。ゲーム実装・検証・リリースは未完了。**
- 作成日: 2026-09-16
- 確定日: 2026-09-19
...
- v1.6.2の実装・テスト・動作確認が完了するまでは、`Nowhere Left to Hide PoC 現行仕様.md`が安定版v1.6.1の正本である。完了後に本確定要件を現行仕様へ反映する。
...
| DOC-01 | v1.6.1で確認された敵隣接による移動制約 / Unit action順序 / infection制約等をAI向けHelpへ明記 | 必須 |
...
- 既存のBranch別`nextArrivalTurn`抽選を維持し、最初に到着予定Turnを迎えたBranchから通常どおり流入させる。初期Checkpointを理由とする流入猶予や段階的な方向解放は設けない。
...
Temporary Housingの人口上限10人をSoft Capから**Hard Cap**へ変更する。定員判定は`workers + infected`で行い、健康住民と感染者の合計を最大10人とする。

- `populationLimitKind = hard`。
- `workers + infected <= 10`を不変条件とする。
- TransferPopulation、Refugee受入先選択、再配置等、全ての人口増加経路で同じ合計定員を検証する。感染による健康住民から感染者への変換は合計定員を変えない。
- 合計10人に達したTemporary Housingは新たな受入先候補から外れる。
...
v1.6.2では、Player所有Facilityの健康人口が感染・戦闘・資源不足などプレイヤーの意図しない事象によって減少した場合、`importantChanges.facilityChanges`へ最低限次を記録する。減少後も稼働を続ける施設を対象から外さない。

- `facilityId`
- healthy population before / after / delta
- infected population before / after（公開可能なPlayer所有Facilityのみ）
- 生産する施設では公開projectionによるresource別output before / after / delta。算出不能なら値を`null`として理由を付ける。生産しない施設ではoutputを`null`とする。
- 変化を起こした公開Eventから判別可能な既存のEvent typeまたはReason Code。非公開情報から原因を補完しない。

プレイヤーが実行したWorker Assignment、TransferPopulation、施設撤収など意図的な配置変更だけによる減員は`facilityChanges`の対象外とする。その操作で別途発生した感染・戦闘などの損失は対象に含める。この項目だけを理由にCrisisを自動発生させない。重要なのは「完全停止しなかったため非意図的な変化が消える」状態をなくすこと。
...
- 上記誤ProduceUnitはPreview、Step、Play-turn、AiSessionのどの入力経路でも`invalid_action_input`で失敗し、Previewはprojectionを返さない。
...
- Unit Productionは拠点ごとに未完了予約1件を上限とし、異なる拠点の予約枠は独立する。Army Baseの無償National Guard報酬は予約枠を消費しない。通常の予約Actionは共通Action予算を1消費する。この現行Coreの挙動を維持し、API metadataとHelpへ明記する（2026-09-19、実装照合後に依頼者確認済み）。
...
- warm実行は再bundleしない。
- 10回warm commandのp95が10秒未満を目標とする。超過は計測結果と原因を調査・記録するが、数値だけでReleaseを不合格にしない。通常経路への`vite-node`再混入、warm時の不要な再bundle、CLI契約不一致はRelease不合格とする。
...
| --- | --- | --- |
| App | 1.6.1 | **1.6.2** |
| Game Rules / State / Config | 11.0.0 | **12.0.0** |
...
| Balanced Agent | 10.0.0 | **11.0.0** |
| Random Agent | 6.0.0 | **6.0.0（維持）** |
| Session Artifact Package | 1.0.0 | **1.0.0（維持）** |
...
3. 全てnormal / active / operational / queue 0。
   既存のBranch別到着予定Turn以降は4方向とも通常流入し、開始時だけの猶予はない。
...
- Temporary Housingは健康住民と感染者の合計が常に10人以下で、感染者による占有も受入可能人数から差し引く。
...
- malformed ProduceUnit `facilityId`版をPreviewすると`invalid_action_input`となり、projectionを返さずDecision・State・RNGを消費しない。
- 同ActionをStep / interactive・finite-plan Play-turn / AiSessionへ渡しても`invalid_action_input`で、Decision・State・RNGを消費しない。
...
- 意図的なWorker Assignment／人口移送だけによる減員は`facilityChanges`へ追加せず、同時に発生した感染・戦闘等の非意図的な損失は記録する。
...
- p95が10秒以上なら原因を調査・記録する。数値超過だけではReleaseを止めず、`vite-node`再混入、warm時の不要な再bundle、CLI契約不一致はReleaseを止める。
...
以下の値・挙動を本版の実装目標とする。数値と公開契約に影響する変更は、本書の更新なしに実装裁量で変えない。

- 「4つの初期Checkpoint」は**4 Branchに各1基、合計4基**を意味する。
- 初期Checkpoint位置はSupply Radius 5の幹線上 `(25,20)/(30,25)/(25,30)/(20,25)`。
- 4方向とも開始時からmanaged / normalとし、既存のBranch別到着予定Turnから通常流入させる。流入猶予は設けない。
...
- Temporary Housingの健康住民と感染者を合計して10人を上限とする。
- Strictは5 Turnから4 Turnへ変更する。感染0等の率は据え置く。
- `importantChanges.facilityChanges`へ記録する健康人口減少は、感染・戦闘等の非意図的な減員に限る。意図的なWorker Assignment／人口移送だけの減員は含めない。
- `ok`の意味は変えず、`accepted`確認をmachine-readableに明文化する。
- 構造が不正なGameActionはPreview・Step・Play-turn・AiSessionで`invalid_action_input`へ統一する。
- 70 Turn性能問題は現技術構成で解決可能と判断し、通常Session入口をbuilt ESMへ変更する。Core全面再設計はしない。
- warm commandのp95 10秒未満は計測・退行調査の目標値であり、超過だけでReleaseを止めない。
