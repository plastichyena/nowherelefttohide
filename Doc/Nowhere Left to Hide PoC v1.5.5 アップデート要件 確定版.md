# Nowhere Left to Hide PoC v1.5.5 アップデート要件 確定版
...
本書は一問一答の回答を反映したv1.5.5の確定要件である。本タスクでは実装・動作検証を行わない。現行仕様は実装・テスト・動作確認が完了するまでv1.5.4のまま維持する。本書の確定後に実装し、完了した要件を現行仕様へ反映する。
...
以降を確定要件とする。第4章の道路は既存のユーザー合意と実装開始値を維持する。係数・座標等は受入条件の範囲で実装時に調整する。今回の判断は第9章に記録する。
...
- 感染者が1人でもいる住宅は民需品生産を停止する。健常者が残っていても生産しない。感染者0に戻り、正常稼働・Supply内・給電中の条件を満たした経済処理から再開する。感染処理と生産の順序は現行を維持し、即時生産処理を追加しない。
...
次表をv1.5.5実装時の契約番号とする。本書作成時にはコードのVersionを変更しない。

| 契約 | v1.5.4 | v1.5.5 |
| --- | --- | --- |
| App | 1.5.4 | 1.5.5 |
| Rules / State / Config | 6.0.0 | 7.0.0 |
| Map ID | fixed-51x51-v3 | fixed-51x51-v4 |
| Save Format | 13 | 14 |
| autosave key末尾 | v13 | v14 |
| Agent API / Observation / Bridge | 11.0.0 | 12.0.0 |
| Artifact Schema | 10.0.0 | 11.0.0 |
| Session / Checkpoint | 7.0.0 | 8.0.0 |
| Balanced Agent | 6.0.0 | 7.0.0 |
| Random Agent | 4.0.0 | 5.0.0 |

- v1.5.4以前の通常Save、autosave、セーブコード、AI Session、Checkpoint、Artifact、検証Replayは非対応とし、移行・変換しない。観戦専用の旧版互換も提供しない。
- 旧Agent契約はVersion mismatchで拒否し、同梱Agent、CLI、Bridge、API文書を新契約へそろえる。戦略Versionは新契約への追従を識別するもので、別途戦略強化を要求しない。
- 実行を伴うSession再開・検証Replayは既存Build identity照合を維持する。観戦は対応App・Artifact・Map・公開SchemaとPackage内部整合を検証し、観戦アプリと記録のBuild IDが異なるだけでは拒否しない。記録内のBuild情報は保持する。
- 道路と実効移動情報を公開マップ・保存・Artifactで一致させる。4.10に従い確定道路配置・生成器Version・Layout Seed・設定識別情報を保持し、読込時に最新生成器で置き換えない。
- ZIPは公開Packageの配布用コンテナであり、公開情報やhashを省略しない。新Artifact Schemaで対応を識別し、内部Package契約も変更する場合はその番号を機械可読APIに公開する。
- 互換性エラー時に旧データ、進行中State、autosaveを上書きしない。旧autosave keyは削除せず、新規開始は nowhere-left-to-hide:auto-save:v14 を使用する。
...
- 旧Save / Session / Checkpoint / Artifactを理由付き・状態不変で拒否する。旧Claude Artifactは旧版拒否の資料に限り、観戦成功条件にしない。
- 複数Seedの通常戦・長期戦でZIP容量と読込・シーク性能を測定する。1,000件以上のDecision、および512 MiB超の大容量記録は耐久試験として扱い、通常容量目標や全端末の成功保証と分離する。512 MiBがZIP実容量・内部展開量のどの指標か明記する。
- 50 MB前後・超過の有効ZIPで容量だけによる一律拒否がないことを確認する。大容量記録は必要範囲読込・メモリ上限・中止・資源不足時のエラー・通常Save保全を検証し、正常再生範囲と再生不能条件を記録する。
- 全速度、コメント表示時間、一時停止、欠損コメント、Turn指定、EndTurnの順序付きログ、末尾停止、タイトル復帰を確認する。
- 人間向けUIとAgentで追加情報の数値と公開範囲が一致する。列挙例にない合法人数の移送も受理し、古いRevision・不正人数・対象不適格時は状態不変とする。
...
| --- | --- |
| 旧Artifact観戦 | 非対応 |
| 旧Save / Session / Checkpoint | 非対応・変換なし |
| 観戦ファイル | 単一ZIP、端末のファイル選択 |
| EndTurn | 処理前後の盤面切替と記録順ログ |
| 再生テンポ | コメント3～8秒・結果1秒、0.5 / 1 / 2 / 4倍、一時停止 |
| 入口・終了 | タイトルのAIリプレイ観戦から入り、タイトルへ戻る。autosave維持 |
| 住宅感染 | 感染者1人以上で生産停止、条件回復後に再開 |
| 人口移送API | 対象組と人数範囲を取得し任意整数人数を指定、Coreで再検証 |
| 長期戦 | 全履歴を対象。1,000件以上・512 MiB超は耐久試験 |
| 人間向け情報 | AIと共通計算。通常画面は要約、詳細パネルで内訳 |
| 対応容量 | 圧縮ZIP 50 MBを暫定目標。超過だけでは拒否せず、事例・制約を記録 |

- 容量は後続の回答を優先する。50 MBはハード上限ではない。10 MB、1 GiB、4 GiBを必須制約にせず、大容量耐久試験を通常ゲームの容量予測に使わない。
- 参考実測: v1.5.4のClaude記録は50 Turn・282 Decision、公開Artifactの594ファイル合計6,166,696 bytes（約5.88 MiB）。内部gzip圧縮済み・外側ZIP化前であり、v1.5.5の容量保証・互換対応を意味しない。場所は output/claude-playtest-20260908/artifact-claude-20260908-s1。
- 道路の採用方針は第4章のとおり。係数・生成座標・性能実測は実装作業として扱い、変更理由と証跡を残す。
- APIフィールド配置、ZIPライブラリ、Worker・キャッシュ構成、文字数換算等は本書の境界と受入条件内の実装裁量とする。未測定値を検証済みの性能保証として記載しない。
- 実装完了後、確定要件と検証記録を現行仕様へ反映する。比較不要になった本書はarchiveへ移す。元ドラフトは履歴資料としてarchiveへ保管し、実装判断は本書を使用する。
