# Nowhere Left to Hide PoC v1.5.2 アップデート要件 ドラフト

- 作成日: 2026-09-05
- 状態: 履歴資料。確定版作成に伴い2026-09-05にarchiveへ移動。実装の根拠には使用しない。
- 後継: [v1.5.2確定要件](../Nowhere%20Left%20to%20Hide%20PoC%20v1.5.2%20アップデート要件%20確定版.md)
...
- 関連: [v1.5.1確定要件](../Nowhere%20Left%20to%20Hide%20PoC%20v1.5.1%20アップデート要件%20確定版.md)。今回の調査では既存の旧資料を参照・移動していない。
...
### 5.2 推奨: ターン単位の対話実行＋複数手入力の代替経路

新しい入口の仮称を`play-turn`とする。コマンド名とJSON schemaは実装前に確定するが、以下の振る舞いを要件とする。
...
- 既存の単発`step`等8コマンドは互換・復旧用に残す。常駐daemonやネットワーク公開を必須にせず、ターン完了時にプロセスを解放する。
...
公開Queryの仮称を`productionCapacity`とし、既存Strategic Forecastに組み込む。全体にRevision、対象Turn、現在人口基準等の算定条件を持たせる。キー名は実装時の型設計で確定する。
...
| `currentWorkerRatedCapacity` | 現在配置済みの人員での定格量。停電等による実際の停止とは区別 |
...
今回は要件策定まで。確定後の別の実装工程で、実装・自動テスト・PCの性能比較・ブラウザ動作確認を完了し、現行仕様へ反映する。SOG05は公開後にユーザーが手動確認し、追加改善が必要なら次回更新の候補にする。
