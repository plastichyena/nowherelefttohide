...
- 基準日: 2026-09-19
- 実装照合日: 2026-09-19
- 直近の反映済み変更要件: `Nowhere Left to Hide PoC v1.6.2 アップデート要件 確定版.md`

本書は現在の実装が従う唯一の正本である。実装、テスト、ヘルプ、保存形式が本書と矛盾する場合は本書を優先する。過去の資料は現行判断には使用しない。v1.6.2の実装と検証状況は18.12に記録し、18.12の差分規則はそれ以前の競合する旧値より優先する。
