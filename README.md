...
- Police Movement 15／National Guard Movement 10、機種別Fuel、Fuel 0 Emergency Movement、Unit別携行軍需品、補給、迎撃、攻撃、反撃、待機、自然回復
...
ゲームルールの正本は [`Doc/Nowhere Left to Hide PoC 現行仕様.md`](Doc/Nowhere%20Left%20to%20Hide%20PoC%20現行仕様.md) です。v1.4.5の実装目標は [`Doc/Nowhere Left to Hide PoC v1.4.5 アップデート要件 確定版.md`](Doc/Nowhere%20Left%20to%20Hide%20PoC%20v1.4.5%20アップデート要件%20確定版.md) です。READMEや変更記録が正本と矛盾する場合は現行仕様を優先します。
...
v1.4.5ではHuman UIとAgentが同じCore Visibilityを使い、Checkpointの新設・移設には対象地点と州都側からの幹線道路全区間の現在視界が必要です。Hidden Zombieは候補や実行を妨害せず、可視Zombieだけが妨害理由になります。Ground VisionはForest／Mountainで遮蔽され、Civilian Drone BaseのAerial Visionは遮蔽を無視します。ObservationはGround／Aerial種別と最新50件の重要Site Eventを返します。初期Normal ZombieはSeedから決定的に配置される25体で、Police Zombie／Soldier Zombieを含む敵は可視時だけ公開します。Checkpointの拒絶Counterと将来Horde増援数は非公開であり、Turn Awayの定性的なTrade-offだけを公開します。Policeは`medium`、National Guardは`large`だけを公開し、正確なRadius、Hidden Enemyの反応数・ID・Target・非可視Spawn位置は公開しません。
...
Session CLIは、外部AIがプロセスや1回の応答をまたいで同じゲームを安全に続けるための永続入口です。通常のSave Format 9とは分離したSessionディレクトリに、Actionごとに更新するActive状態、公開`trace.ndjson`、既定5完了Turnごとの履歴Checkpointを保存します。AIの意思決定に使えるのはCLIが返す公開Observation、Legal Actions、Step結果、公開Event、自身のPublic Decision Logだけで、Private Checkpoint内の完全GameState、RNG、Rejected CounterなどのHidden情報は再開専用です。
...
ゲームルール内では `Math.random()` を使いません。`SeededRng` のスナップショット（Seed、状態、呼出回数、アルゴリズム）もJSON化し、同じVersion・Build・Config・Map・Seed・Action列から同じ結果を得られるようにします。App/Release Versionは `1.4.5`、Game Rules / GameState / Configは `2.5.0`、Fixed Mapは `fixed-51x51-v1`、Agent / Observation / Browser Bridge APIは `6.1.0`、Artifact Schemaは `5.0.0`、Checkpoint／Session Schemaは`2.0.0`、Balanced Agentは`4.4.0`、Random Agentは`2.3.0`です。v1.4.4以前のSave、Replay、Artifact、Session、Checkpointは変換せず拒否します。
...
確定したActionまたはターン終了時にローカル領域へ自動保存します。タイトル画面から続きのゲームを読み込めます。App／Release `1.4.5`、Game Rules／State／Config `2.5.0`、Save Format `9`を使用します。Save 9はFixed Map 51×51、Seed付き初期Zombie、Rejected Counter、Police／Soldier Zombie、Checkpoint初回Build履歴、PRNG、Config完全コピーを検証し、導出値は復元時に再計算します。v1.4.4以前のSave、Replay、Artifact、Session、Checkpointは変換せず、現在状態と元データを変更しないままVersion不一致として拒否します。
...
npx --no-install vite-node --script src/agent/sim-cli.ts --agent=balanced --games=300 --seed=1 --summary-only --out=output/simulations/v1.4.5-balanced-300 --overwrite
npm run build
...
`.github/workflows/v140-release-validation.yml` はファイル名を維持したv1.4.5手動Release検証として、v1.4.4 stable commitの同Seed／同上限の基準Batch、標準Random 100ゲーム、標準Balanced 300ゲームを実行します。Balanced検証はSeed `1..100`、`101..200`、`201..300`の3並列Jobに分け、比較可能なJSON／CSV ReportをArtifactへ保存します。
...
Pages公開後は実ブラウザでゲームURLを開き、`window.NLTH`をSeed 1でGame Overまで実行します。`appVersion` 1.4.5、各6.1.0 API、Artifact 5.0.0、51×51 Map、Checkpoint偵察ゲート、Ground／Aerial Vision、Turn Awayの公開境界、重要Site Event、`verificationEvents`とHidden Spawn／Rejected Counter情報の非公開、Action列Replay一致を確認します。勝利は合格条件ではありません。
