...
This repository is designed so an external AI/LLM can play the same game rules as a human without reading private `GameState` internals. The current release is v1.3.2.
...
- Checkpoint observations show screening pools, current policy, and Core-generated `checkpointPositionCandidates` for every branch road tile. Read each candidate's `legal` and `reasonCode` instead of reconstructing checkpoint rules. Hidden enemies never make a candidate illegal and are not revealed through candidate differences. `getApiInfo().rules.checkpointPolicies` gives the static screening time, acceptance, infection chance, and infected-population rate for Pass through, Normal, and Strict. These are trade-offs; no policy is always correct.
...
The intended information boundary is the same one used by the built-in Agent platform and Human UI: public Observation plus currently legal actions. App `1.3.2` uses Game Rules, Agent, Observation, Bridge, and Artifact contracts `1.4.1`, Save Format `3`, Balanced Agent `3.0.0`, and Random Agent `1.2.0`.
