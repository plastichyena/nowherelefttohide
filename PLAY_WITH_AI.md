...
This repository is designed so an external AI/LLM can play the same game rules as a human without reading private `GameState` internals. The current release is v1.6.2.
...
## v1.6.2 decision aids and query discovery
...
Recruitment has one pending reservation **per facility**, with independent slots at different facilities. Ordinary recruitment consumes a global action. The free Army Base National Guard reward consumes no reservation slot.

New games begin with four active Normal checkpoints at radius 5; all additional construction and relocation costs 25 Civilian Goods. City-1 and Military Factory-1 are already owned. There are 110 healthy civilians, seven regular player units (four Police, one Riot Police, one Recon and one National Guard), stocks 330/355/175/192, and exactly 50 zombies (40 Normal, 4 Gas, 4 Hunter, 2 Screamer).

Temporary Housing accepts at most 10 healthy plus infected residents. Strict screens 20 people in 4 turns, accepts 50% with zero infection, and is still slower than Normal. There is no fixed safe population: compare runway, `guaranteed_resource_defeat`, production capacity and queue demand before admitting more. Keep defenders for Capital, Power Plants and principal Food/Civilian Goods producers when threats approach. Police/Riot Police can suppress Capital infection without civilian damage.
...
The Session directory includes a private Save Format 19 checkpoint state solely so the runtime can resume deterministically. Session/Checkpoint Schema 13 stores immutable generation data, persistent request IDs, compressed/chunked public payloads, compact Decision records, lossless patches, and hash-chain references so a long history is not repeatedly materialized in ordinary commands. v1.6.1 and earlier AI Session, Checkpoint, Artifact, and Replay data are not migrated; start a new v1.6.2 AI Session and retain old data for use with its old release. Do not inspect or use private state, RNG state, hidden enemies/targets, Rejected Refugee counters, exact neutral-survivor counts, Screamer radius, or non-public configuration for decisions. The public Decision Log, CLI JSON, and Artifact Schema 16.0.0 output are the fair-play record; their Decision hash chain detects accidental damage or inconsistency but is not a cryptographic authenticity guarantee against someone rewriting every file coherently.
...
- Standard fixed Waves spawn at Turn 10 (1 direction, 5 Horde + 3 non-Horde slots), Turn 20 (2, 3 + 5), Turn 35 (1, 8 + 7), Turn 50 (3, 5 + 7), and Turn 70 (4, 8 + 8 Final). Non-Horde weights are Normal 65, Police 10, Soldier 10, Riot 5, Hunter 5, and Screamer 5; the last two Waves add Gas 5 and use Normal 60. Riot, Hunter, and Gas are independently capped at one per direction per Wave. Warning Lead is 2 turns and exposes only slot count and possible types; the concrete draw is first public after Spawn. Do not infer pre-warning directions, Spawn Group IDs, exact positions, or hidden counts.
...
- Normal movement Fuel is twice the existing Hex-count table. Police and Riot Police carry 24 Fuel and 10 Military Goods and pay 2 Military Goods for a range-1 attack. National Guard carries 44 Fuel and 40 Military Goods and pays 2 at range 1 or 4 at range 2. A range-1 shortage attack may consume the remaining 0 or 1 Military Good and deals 20% attack, minimum 1; Recon has no shortage attack. Natural recovery is 5% after combat and 10% after rest, rounded up.
...
- A Police, Riot Police, or National Guard unit stationed at an infected location contains internal spread regardless of its carried Military Goods. Automatic suppression can use every remaining Attack Charge that can pay its Military Goods cost. Police/Riot suppression has no civilian damage; National Guard suppression is stronger but inflicts civilian damage at 50% of suppressed infection. Treat that population cost as part of the EndTurn decision. `endTurnRisk` summarizes ready Units, remaining charges, legal attacks, and uncontained sites without changing legality or state.
- Use `baseRange`, `effectiveRange`, attack previews, and shortage reasons rather than assuming a unit's range. National Guard distance 2 is available only while it can pay 4 Military Goods; the removed global `militarySupplyAvailable` state is not part of the current API.
...
- `BuildConstructibleFacility` creates Simple Farm (Civilian Goods 25), Civilian Drone Base (50), Temporary Housing (25), or Wind Power Plant (150) on a Core-listed, currently visible, supplied, empty Plain Hex without road, entrance, reserve, facility, checkpoint, barbed wire, Player Unit, or visible Zombie. Unseen destinations return `constructible_not_visible` regardless of hidden walls or enemies. Construction completes next Player Turn. Read Core candidates for legality.
- Simple Farm has Power Mode `none`, Worker Capacity 10, and produces Food 5 per worker without Electricity. Its player-build limit is the road-branch count (4 on the fixed map); query API or construction candidates rather than assuming it is unlimited. Civilian Drone Base has Power Mode `required`, needs 5 Electricity, and provides Vision 3 per worker when powered. Both preserve existing workers/functions outside Supply, but cannot gain workers there.
...
## v1.6.2 Wave and housing decisions
...
The intended information boundary is the same one used by the built-in Agent platform and Human UI: public Observation plus currently legal actions. App `1.6.2` uses Game Rules `12.0.0`, Agent/Observation/Browser Bridge API `17.0.0`, Fixed Map `fixed-51x51-v7`, Save Format `19`, Artifact Schema `16.0.0`, Checkpoint/Session Schema `13.0.0`, Play-turn Protocol `1.1.0`, Balanced Agent `11.0.0`, and Random Agent `6.0.0`. Artifact Schema 16.0.0 packages public Wave/Warning/Site Event, Screamer/Army Base/Oil Field state, production-capacity and support-headroom state, Metrics, a lossless public Decision Log, request identity, and lineage without private Checkpoint state. v1.6.1 and earlier AI Replay, Artifact, Session, Checkpoint, and normal Save data are rejected without conversion or overwrite.
...
## v1.6.2 parameter queries and ZIP spectator
...
Only current visible walls are public. Version 1.6.1 and older saves, checkpoints, sessions and artifacts are rejected without migration or rewriting old autosaves.
