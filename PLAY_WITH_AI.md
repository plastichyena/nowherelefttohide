...
This repository is designed so an external AI/LLM can play the same game rules as a human without reading private `GameState` internals. The current release is v1.6.0.
...
## v1.6.0 decision aids and query discovery
...
The Session directory includes a private Save Format 17 checkpoint state solely so the runtime can resume deterministically. Session/Checkpoint Schema 11 stores immutable generation data, persistent request IDs, compressed/chunked public payloads, compact Decision records, lossless patches, and hash-chain references so a long history is not repeatedly materialized in ordinary commands. v1.5.7 and earlier AI Session, Checkpoint, Artifact, and Replay data are not migrated; start a new v1.6.0 AI Session and retain old data for use with its old release. Do not inspect or use private state, RNG state, hidden enemies/targets, Rejected Refugee counters, or non-public configuration for decisions. The public Decision Log, CLI JSON, and Artifact Schema 14.0.0 output are the fair-play record; their Decision hash chain detects accidental damage or inconsistency but is not a cryptographic authenticity guarantee against someone rewriting every file coherently.
...
- Four neutral Oil Fields support the national Refinery Allowance. It starts at 5,000; each healthy worker at an operating Oil Field adds 100 permanent credits during EndTurn, and each Refinery spends one credit per Fuel produced. Oil Fields use no power or Fuel and produce no stored resource directly. Same-turn Oil credit can fund later Refinery production but cannot retroactively fund that turn's earlier power generation.
...
## v1.6.0 Wave and housing decisions
...
The intended information boundary is the same one used by the built-in Agent platform and Human UI: public Observation plus currently legal actions. App `1.6.0` uses Game Rules `10.0.0`, Agent/Observation/Browser Bridge API `15.0.0`, Fixed Map `fixed-51x51-v5`, Save Format `17`, Artifact Schema `14.0.0`, Checkpoint/Session Schema `11.0.0`, Balanced Agent `9.0.0`, and Random Agent `6.0.0`. Artifact Schema 14.0.0 packages public Wave/Warning/Site Event, Gas/Army Base/Oil Field state, production-capacity and support-headroom state, Metrics, a lossless public Decision Log, request identity, and lineage without private Checkpoint state. v1.5.7 and earlier AI Replay, Artifact, Session, Checkpoint, and normal Save data are rejected without conversion or overwrite.
...
## v1.6.0 parameter queries and ZIP spectator
