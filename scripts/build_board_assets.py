...
V153_SOURCE_FILES = {
    # v1.5.3 assets are generated by the built-in ImageGen tool.  The source
    # root is the generated-image directory; the UUID filenames are retained
    # here so the exact approved source can be rebuilt without changing the
    # runtime registry paths.
    "units/unit_hunter_zombie.png": "exec-0e153f36-a186-413e-9ebb-808dd9364d13.png",
    "units/unit_gas_zombie.png": "exec-241e6709-0a91-4c48-8942-ffd4b02f2bb6.png",
...
def build_v153(source_root: Path, output_root: Path) -> None:
    """Post-process the v1.5.3 Hunter, Gas, and Army Base concepts."""
    for relative, source_name in V153_SOURCE_FILES.items():
        destination = output_root / relative
        destination.parent.mkdir(parents=True, exist_ok=True)
        source = Image.open(source_root / source_name)
        bounds = (202, 202) if relative.startswith("units/") else (218, 218)
        y_offset = 0 if relative.startswith("units/") else 3
        contain(source, bounds, y_offset=y_offset).save(
            destination,
            optimize=True,
            compress_level=9,
        )
