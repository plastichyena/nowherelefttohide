...
export function showReplay(root: HTMLElement, locale: Locale, exit: () => void): void {
  ...
  const title=document.createElement('summary');title.textContent=ja?'v1.6.3 凡例・ルール':'v1.6.3 legend and rules';legend.append(title);
  ...
  const draw = () => {
    ...
    for(const tile of pkg.map.tiles){const p=transform(tile.q,tile.r);if(p.x < -scale || p.x>rect.width+scale || p.y < -scale || p.y>rect.height+scale)continue;ctx.beginPath();for(let k=0;k<6;k++){const angle=(60*k-30)*Math.PI/180;const x=p.x+Math.cos(angle)*scale,y=p.y+Math.sin(angle)*scale;if(k===0)ctx.moveTo(x,y);else ctx.lineTo(x,y);}ctx.closePath();ctx.fillStyle=tile.terrain==='forest'?'#355447':tile.terrain==='mountain'?'#5d615d':tile.terrain==='water'?'#345569':'#6b7053';ctx.globalAlpha=visible.has(hexKey(tile))?1:.35;ctx.fill();if(tile.terrain==='water'&&zoom>=1.5){ctx.save();ctx.clip();sprite(ctx,BOARD_ASSET_REGISTRY.terrain.water,p.x,p.y,scale*2);ctx.restore();}ctx.globalAlpha=1;}
    for(const e of roadEdges(pkg.map.roads!)){const a=transform(e.a.q,e.a.r),b=transform(e.b.q,e.b.r);ctx.strokeStyle=e.role==='trunk'?'#b5a98d':e.role==='collector'?'#968e7b':'#777467';ctx.lineWidth=Math.max(.5,scale*(e.role==='trunk'?.28:e.role==='collector'?.16:.09));ctx.beginPath();ctx.moveTo(a.x,a.y);ctx.lineTo(b.x,b.y);ctx.stroke();if(zoom>=1.5)for(const [tile,at,other] of [[e.a,a,b],[e.b,b,a]] as const){if(pkg.map.tiles.find(t=>t.q===tile.q&&t.r===tile.r)?.terrain!=='water')continue;ctx.save();ctx.translate(at.x,at.y);ctx.rotate(Math.atan2(other.y-at.y,other.x-at.x));sprite(ctx,BOARD_ASSET_REGISTRY.overlays.bridge,0,0,Math.hypot(other.x-at.x,other.y-at.y));ctx.restore();}}
    ...
    for(const w of current.observation.barbedWire) { const p=transform(w.position.q,w.position.r);ctx.strokeStyle='#e4e9ed';ctx.lineWidth=2;ctx.strokeRect(p.x-scale*.8,p.y-scale*.8,scale*1.6,scale*1.6);ctx.fillStyle='#fff';ctx.font='10px sans-serif';ctx.fillText(`W ${w.hp}/10`,p.x-scale,p.y-scale); }
