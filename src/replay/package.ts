...
export class ReplayPackage {
  ...
  async open():Promise<this> {
    ...
    if(m.gameRulesVersion!==GAME_RULES_VERSION || String(m.saveFormatVersion)!==SAVE_FORMAT_VERSION || m.agentApiVersion!==AGENT_API_VERSION || m.bridgeApiVersion!==BRIDGE_API_VERSION || m.appVersion!==APP_VERSION || m.artifactSchemaVersion!==ARTIFACT_SCHEMA_VERSION || m.observationApiVersion!==OBSERVATION_API_VERSION || m.mapId!=='fixed-51x51-v5' || m.packageVersion!==SESSION_ARTIFACT_PACKAGE_VERSION || m.sessionSchemaVersion!==SESSION_SCHEMA_VERSION) fail('Unsupported replay version: v1.6.0 public Artifact required; start a new v1.6.0 game/Session (旧Replay非対応、新規v1.6.0 Sessionを開始してください)');
