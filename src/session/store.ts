...
export class SessionStore {
  ...
  private readDescriptor(directory: string): SessionDescriptor {
    ...
    if (descriptor.sessionSchemaVersion !== SESSION_SCHEMA_VERSION || descriptor.checkpointSchemaVersion !== CHECKPOINT_SCHEMA_VERSION) throw new SessionError('session_version_mismatch', 'Session or Checkpoint Schema Version is unsupported; start a new v1.6.2 Session. 旧データは変換・上書きしません。');
  ...
  private readCheckpointMetadata(directory: string, descriptor: SessionDescriptor, path: string): SessionCheckpointMetadata {
    ...
    if (metadata.checkpointSchemaVersion !== CHECKPOINT_SCHEMA_VERSION || metadata.sessionSchemaVersion !== SESSION_SCHEMA_VERSION || metadata.sessionId !== descriptor.sessionId) throw new SessionError('checkpoint_version_mismatch', 'Checkpoint does not match Session Schema; start a new v1.6.2 Session.');
