...
export function projectContextHandoff(observation: Omit<AgentObservation, 'map'>, identity: HandoffIdentity, history: ContextHandoffHistory) {
  ...
  return structuredClone({
    ...
    authoritativeState: {
      ...
      productionLedger:observation.productionLedger, resources: observation.resources, population: observation.population, capital: observation.facilities.find(f => f.type === 'capital'),
      ...
      nuclearObjective: observation.nuclearObjective, facilityObjectives: observation.facilityObjectives, militaryDrone: observation.militaryDrone, refineryAllowance: observation.refineryAllowance,
      ...
      units: bounded(observation.units.map(u => ({ id: u.id, type: u.type, flightState:u.flightState, movementDomain:u.movementDomain, cargoUnitId:u.cargoUnitId, cargoUnitType:u.cargoUnitType, transportedByUnitId:u.transportedByUnitId, canTakeOff:u.canTakeOff, canLand:u.canLand, canBoard:u.canBoard, canDisembark:u.canDisembark, canRefuel:u.canRefuel, canResupplyMilitaryGoods:u.canResupplyMilitaryGoods, takeOffReasonCode:u.takeOffReasonCode, landReasonCode:u.landReasonCode, supplyReasonCode:u.supplyReasonCode, mode:u.mode, modeLockedUntilTurn:u.modeLockedUntilTurn, capabilities:u.capabilities, production:u.production, position: u.position, hp: u.hp, attackChargesRemaining: u.attackChargesRemaining, currentFuel: u.currentFuel, currentMilitaryGoods: u.currentMilitaryGoods })), CONTEXT_HANDOFF_LIMITS.units, 'units'),
      visibleEnemies: bounded(observation.zombies.map(u => ({ id: u.id, type: u.type, movementDomain: u.movementDomain, position: u.position, hp: u.hp, maxHp: u.maxHp, attack: u.attack, movement: u.movement, baseMovement: u.baseMovement, appliedMovementBonus: u.appliedMovementBonus, effectiveMovement: u.effectiveMovement, range: u.range, vision: u.vision, attackChargesRemaining: u.attackChargesRemaining, maxAttackCharges: u.maxAttackCharges, isFinalWaveMember: u.isFinalWaveMember })), CONTEXT_HANDOFF_LIMITS.visibleEnemies, 'full-snapshot'),
