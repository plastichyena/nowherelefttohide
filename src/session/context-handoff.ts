...
export function projectContextHandoff(observation: Omit<AgentObservation, 'map'>, identity: HandoffIdentity, history: ContextHandoffHistory) {
  ...
  return structuredClone({
    ...
    authoritativeState: {
      ...
      units: bounded(observation.units.map(u => ({ id: u.id, type: u.type, mode:u.mode, modeLockedUntilTurn:u.modeLockedUntilTurn, capabilities:u.capabilities, production:u.production, position: u.position, hp: u.hp, attackChargesRemaining: u.attackChargesRemaining, currentFuel: u.currentFuel, currentMilitaryGoods: u.currentMilitaryGoods })), CONTEXT_HANDOFF_LIMITS.units, 'units'),
