...
describe('Session Schema 7 bounded storage and queries', () => {
  ...
  it('reconstructs a bounded history page from the nearest periodic snapshot', () => {
    ...
    const summaryPage = api.query('history', { target: 'history', expectedRevision: 55, pageSize: 2, filters: { fromDecision: 53, toDecision: 55 } });
    expect(summaryPage).toMatchObject({ count: 2, total: 3, hasMore: true });
    expect(summaryPage.items?.[0]).not.toHaveProperty('observationBefore');
    expect(summaryPage.items?.[0]).not.toHaveProperty('observationAfter');
    const page = api.query('history', { target: 'history', expectedRevision: 55, pageSize: 2, filters: { fromDecision: 53, toDecision: 55, includeSnapshots: true } });
    ...
    const final = api.query('history', { target: 'history', expectedRevision: 55, cursor: page.nextCursor!, pageSize: 2, filters: { fromDecision: 53, toDecision: 55, includeSnapshots: true } });
...
describe.runIf(process.env.NLTH_SESSION_DAILY === '1').sequential('Session Schema 7 daily 1000-decision chain', () => {
