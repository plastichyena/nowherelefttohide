...
export function createAgentApiInfo(
  ...
  return cloneJson({
    ...
    publicInformation: [
      ...
      'Every Human Unit has zero fixed Military Goods upkeep; attacks, suppression and refills still apply. Military Factory input/output per operating worker comes from rules.production.militaryFactoryPerWorker.',
      ...
      'Unowned currentWorkers, healthyPopulation and healthyWorkers are null (unknown, never zero); availablePopulation counts eligible owned cities only. Production/recovery reasons never reveal hidden survivors.',
      'All Human Unit population and conscripted reservations, including cargo, cost unitFoodConsumption Food per person; civilians/base workers/healthy queues use civilianFoodConsumption. No military starvation deaths.',
      `All nine Zombie types start with base MP on their first eligible pursuit phase, then gain +${config.zombiePursuitMovementBonus} MP on consecutive phases targeting visible_population, inherited_horde, wave_capital or capital. Switching between these targets preserves pursuit. Noise/idle immediately clears both memory and bonus; reacquisition starts at base MP. This adds no random draw.`,
      'Visible zombies expose baseMovement, appliedMovementBonus and effectiveMovement from the latest resolved phase; no private target/continuity memory or guaranteed next-phase prediction.',
      'Reference routes do not promise a legal Move. At the destination actionRequired=false: do not send Move. Refresh queries at the current revision after every action; read interception/counterattack charges from the Move result. Adjacent enemies do not prohibit withdrawal.',
      'Checkpoint candidates expose routeVisibility.missingVisibleHexes from public roads/current vision; explored tiles alone do not satisfy the corridor rule.',
      'Owned facilities publish actual population and separate zombieTargetValue; Wind is a target value 5 but has no civilian population.',
    ...
    rules: {
      ...
      production: {
        civilianFoodConsumption: config.economy.populationConsumption.food,
        unitFoodConsumption: config.economy.unitFoodConsumption,
        militaryFactoryPerWorker: cloneJson(config.facilities.militaryFactory.production),
