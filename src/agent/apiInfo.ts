...
export function createAgentApiInfo(
  ...
  const reconTeam = units.reconTeam ?? {};
  ...
  return cloneJson({
    ...
    actionContracts: {
      ...
      TransferPopulation: { required: ['fromFacilityId', 'toFacilityId', 'people'], example: { type: 'TransferPopulation', fromFacilityId: 'capital', toFacilityId: 'city-1', people: 7 }, conditions: ['query population-transfers', 'inclusive positive integer range', 'Core revalidates at execution; enumerated examples are not exhaustive'] },
      SetCheckpointPolicy: { required: ['branchId', 'policy'], example: { type: 'SetCheckpointPolicy', branchId: 'north', policy: 'strict' }, conditions: ['branch policy', 'Normal/Strict rejection may strengthen future participating Waves before Final roster freeze'] },
      TurnAwayCheckpointRefugees: { required: ['checkpointId', 'count'], example: { type: 'TurnAwayCheckpointRefugees', checkpointId: 'checkpoint-north', count: 1 }, conditions: ['waiting only; screening/approved excluded', 'branch action budget', 'may strengthen future Waves before Final roster freeze'] },
    },
    methodSchemas: {
    ...
    publicInformation: [
      ...
      'The enemy list contains only currently visible Normal, Horde, Police, Soldier, Riot, Hunter, Gas, and Screamer Zombies; hidden enemies are omitted.',
    ...
    rules: {
      ...
      recon: {
        recruitAttack: getNumber(reconTeam, 'recruitAttack', 9),
        hp: getNumber(reconTeam, 'hp', 25),
        movement: getNumber(reconTeam, 'movement', 10),
        range: getNumber(reconTeam, 'range', 6),
        vision: getNumber(reconTeam, 'vision', 10),
        population: getNumber(reconTeam, 'population', 5),
        maxFuel: getNumber(reconTeam, 'maxFuel', 44),
        maxMilitaryGoods: getNumber(reconTeam, 'maxMilitaryGoods', 40),
        attackMilitaryGoodsCost: 6,
        noiseClass: 'medium',
        productionFacilities: ['capital', 'armyBase'],
      },
      recovery: {
      ...
      ranges: {
        ...
        gasZombie: { baseRange: config.units.gasZombie.range },
        screamerZombie: { baseRange: config.units.screamerZombie.range },
      },
      terrain: {
      ...
      checkpointPolicies: {
        ...
        deny: {
          turns: policies.deny.turns,
          acceptanceRate: policies.deny.workerRate,
          infectionBatchRate: policies.deny.infectionRate,
          infectedPopulationRate: policies.deny.infectionPopulationRate,
        },
      },
