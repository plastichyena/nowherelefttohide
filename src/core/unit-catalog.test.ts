...
describe('closed existing unit catalog', () => {
  it('preserves validation, production and weighted draw orders', () => {
    expect(UNIT_TYPES).toEqual(['police', 'nationalGuard', 'riotPolice', 'reconTeam', 'specialForces', 'packZombie', 'zombie', 'hordeZombie', 'policeZombie', 'soldierZombie', 'riotZombie', 'hunterZombie', 'gasZombie', 'screamerZombie']);
    expect(HUMAN_UNIT_TYPES).toEqual(['police', 'nationalGuard', 'riotPolice', 'reconTeam', 'specialForces']);
