...
export function gasAttackPreview(state: Readonly<GameState>, target: Readonly<UnitState>, directDamage: number): GasAttackPreview | null {
  ...
  while (queue.length) {
    ...
    for (const u of hit) u.hp -= Math.min(u.hp, terrainAdjustedDamage(state, u.unit, (u.unit.isPlayerUnit ? state.config.units.gasZombie.explosionDamage : state.config.units.gasZombie.explosionZombieDamage)).finalDamage);
