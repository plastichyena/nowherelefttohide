...
describe('v1.5.3 integrated rules',()=>{
 ...
 it('accepts unpowered urban recruitment, retains the payment, then completes when power returns',()=>{
  ...
  const completed=engine.step({type:'EndTurn'});expect(completed.error?.message??null).toBeNull();expect(completed.state.pendingUnitProductions).toHaveLength(0);expect(completed.state.units.filter(u=>u.type==='nationalGuard')).toHaveLength(2);expect(completed.state.units.filter(u=>u.type==='nationalGuard').at(-1)).toMatchObject({proficiency:'recruit',currentMilitaryGoods:40});expect(completed.state.facilities.find(f=>f.type==='capital')?.workers).toBe(31);
 });
