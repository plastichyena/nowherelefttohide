...
describe('v1.5.3 integrated rules',()=>{
 it('selects every Army Base candidate deterministically without initializing engines', () => {
  const selected = Array.from({length: 40}, (_, i) => selectArmyBasePosition(new SeededRng(i + 1)));
 ...
 it('fires continuously at distance zero until the refreshed worker shots are exhausted', () => {
  ...
  s.facilities.find(f => f.type === 'airBase')!.armyBase!.militaryGoods = 0;
 ...
 it('captures a neutral Army Base after empty occupation and completes recovery next Player Turn', () => {
  ...
  b.workers = 0;
  b.earlyCaptureSurvivorStatus = 'lost';
  const neighbors = hexNeighbors(b.position).filter((position) => !s.facilities.some((facility) => facility.position.q === position.q && facility.position.r === position.r));
  ...
  expect(capturedBase.armyBase?.reward).toBe('expired');
  expect(captured.state.units.filter((unit) => unit.type === 'nationalGuard')).toHaveLength(1);
  const restored = engine.step({ type: 'EndTurn' });
  expect(restored.state.facilities.find((facility) => facility.id === b.id)).toMatchObject({
    owner: 'player', status: 'owned', operationalStatus: 'operational', workers: 0,
  });
 ...
 it('accepts unpowered urban recruitment, retains the payment, then completes when power returns',()=>{
  ...
  const restored=engine.getState() as GameState;restored.facilities.find(f=>f.id==='power-plant-1')!.workers=10;for(const f of restored.facilities)if(f.type==='powerPlant'||f.type==='windPowerPlant')f.operationalStatus=f.owner==='player'?'operational':f.operationalStatus;load(engine,restored);
  const completed=engine.step({type:'EndTurn'});expect(completed.error?.message??null).toBeNull();expect(completed.state.pendingUnitProductions).toHaveLength(0);expect(completed.state.units.filter(u=>u.type==='nationalGuard')).toHaveLength(2);expect(completed.state.units.filter(u=>u.type==='nationalGuard').at(-1)).toMatchObject({proficiency:'recruit',currentMilitaryGoods:40});expect(completed.state.facilities.find(f=>f.type==='capital')?.workers).toBe(paid.facilities.find(f=>f.type==='capital')?.workers);
 });
 it('never substitutes base workers for city recruitment population',()=>{const cfg=config();cfg.checkpoint.initialSupplyRadius=8;const engine=new GameEngine(3,cfg);const {s,b}=owned(engine,10);const capital=s.facilities.find(f=>f.type==='capital')!;s.population.cumulativeDeaths+=capital.workers-9;capital.workers=9;load(engine,s);const before=engine.getState();expect(engine.step({type:'ProduceUnit',unitType:'nationalGuard',destination:b.position}).error?.code).toBe('insufficient_population');expect(engine.getState()).toEqual(before);});
