...
/** The sole game-rules version accepted by v1.6.7 saves. */
...
/** v1.6.7 never writes to an earlier autosave namespace. */
export const DEFAULT_AUTOSAVE_KEY = 'nowhere-left-to-hide:auto-save:v24';
/** Read-only compatibility probe for the immediately preceding autosave namespace. */
export const LEGACY_AUTOSAVE_KEY = 'nowhere-left-to-hide:auto-save:v23';
...
const GAME_EVENT_TYPES = [
  ...
  'facility_captured',
  'survivors_rescued',
  'aircraft_state_changed', 'aircraft_boarded', 'aircraft_disembarked', 'aircraft_emergency_landing',
  'military_drone_launched', 'military_drone_expired', 'facility_objective_updated',
  'survivors_expired',
  'workers_assigned',
...
const STATISTIC_INTEGER_FIELDS = [
  ...
  'screamerScreams',
  'packZombiesSpawned', 'packZombiesKilled', 'specialForcesReanimations',
  'starvationDeaths', 'screeningInfections', 'waitingInfections', 'livingConditionInfections',
] as const;
...
function incompatibilityError(found: unknown, subject: string): string {
  return `${subject} is incompatible with v1.6.7; v1.6.6 or earlier data requires a new v1.6.7 game / Game Rules ${CURRENT_GAME_VERSION} / Save Format ${SAVE_FORMAT_VERSION} (found ${String(found)}; expected ${CURRENT_GAME_VERSION}). 現在のゲーム状態は変更されません。旧Saveは変換・削除・上書きされません。`;
}
...
function validateV144Shape(state: Record<string, unknown>, errors: string[]): void {
  ...
  if (!Array.isArray(facilities)) {
    ...
    for (const [index, facility] of facilities.entries()) {
      ...
      if ((facility.type === 'armyBase' || facility.type === 'airBase')) {
        ...
        errors.push(`${path}.armyBase is only valid for a military base`);
      }
    }
  }
  ...
  if (!Array.isArray(checkpoints)) {
    ...
    for (const [index, checkpoint] of checkpoints.entries()) {
      ...
      if (typeof checkpoint.waitingRiskPercent !== 'number' || !Number.isFinite(checkpoint.waitingRiskPercent) || checkpoint.waitingRiskPercent < 0 || checkpoint.waitingRiskPercent > 12) errors.push(`${path}.waitingRiskPercent is invalid`);
  ...
  if (!Array.isArray(pending)) {
    ...
    for (const [index, order] of pending.entries()) {
      ...
      const facility = Array.isArray(facilities)
        ...
        : undefined;
      if (isRecord(facility) && (facility.type === 'armyBase' || facility.type === 'airBase')) {
        const productionConfig = isRecord(config) && isRecord(config.units) ? config.units[order.unitType as string] : undefined;
        if (isRecord(productionConfig) && !(productionConfig.recruitmentFacilityTypes as unknown[])?.includes(facility.type)) errors.push(`${path}.unitType is ineligible for this military base`);
        ...
        errors.push(`${path}.powerReady is only valid for a military base reservation`);
      }
    }
  }
...
function validateStatisticsShape(value: unknown, errors: string[], path: string): void {
  ...
  for (const field of ['hordeSpecialSpawnedByType', 'finalSpecialZombiesSpawnedByType'] as const) {
    ...
    if (!isRecord(counters)) {
      ...
      for (const unitType of ['policeZombie', 'soldierZombie', 'riotZombie', 'hunterZombie', 'gasZombie', 'screamerZombie', 'packZombie'] as const) {
        if (!isInteger(counters[unitType])) errors.push(`${path}.${field}.${unitType} is invalid`);
      }
    }
  }
...
export class AutoSaveStore {
  ...
  /** Clears only the current v1.6.7/v24 key; legacy data is deliberately preserved. */
  clear(): void {
    try {
