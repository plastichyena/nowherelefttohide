...
describe('release seed validation gates', () => {
  it('accepts exactly 100 completed games per agent across all shards', () => {
    ...
    expect(validateReleaseSeedCoverage(reports)).toEqual({ games: 200, seedsPerAgent: 100, finalHordeReachableRuns: 100 });
  });
  ...
  it('requires Balanced to reach the Final Horde across the combined coverage', () => {
    const reports = ['random', 'balanced'].flatMap(agent => RELEASE_SEED_STARTS.map(start => report(agent, start)));
    ...
    expect(() => validateReleaseSeedCoverage(reports)).toThrow(/Final Horde/);
